...
/**
 * OutlookInboxListBinder
 *
 * Lists inbox emails for a given user.
 * Supports server-side date range filtering via hidden DataList filter columns:
 *   - start_filter  → receivedDateTime ge
 *   - end_filter    → receivedDateTime le
 * Subject filtering is applied client-side after fetching.
 * Uses app-only (client credentials) access token.
 */
public class OutlookInboxListBinder extends DataListBinderDefault {

    private static final String TAG        = "OutlookInboxListBinder";
    ...
    @Override public String getLabel()       { return "Outlook Inbox List (Date+Subject Filter)"; }
    @Override public String getDescription() {
        return "Lists inbox emails for a user. " +
    ...
    public String getPropertyOptions() {
        return AppUtil.readPluginResource(getClassName(),
                "/properties/OutlookInboxListBinder.json", null, true, null);
    }
    ...
    public DataListColumn[] getColumns() {
        return new DataListColumn[]{
                ...
                new DataListColumn("receivedDate",      "Received Date",       true),
                new DataListColumn("hasAttachments",    "Attachments",         true),
                new DataListColumn("isRead",            "Is Read",             true),
...
//            LogUtil.info(TAG, "[getCount] URL: " + url);
    ...
    private String buildInboxUrl(String user, String dateFrom, String dateTo,
        ...
        if (StringUtils.isNotBlank(dateFrom)) {
            filter.append("receivedDateTime ge ").append(dateFrom);
        }
        ...
        params.add("$orderby=" + enc("receivedDateTime desc"));
        params.add("$select=" + enc(
                "id,subject,from,toRecipients,receivedDateTime," +
                        "hasAttachments,isRead,importance,conversationId," +
                        "internetMessageId,bodyPreview"
        ));
        if (top  > 0) params.add("$top="  + top);
        if (skip > 0) params.add("$skip=" + skip);

        return GRAPH_BASE + enc(user) + "/mailFolders/Inbox/messages?" +
                String.join("&", params);
    }
    ...
    private FormRow toFormRow(JsonNode msg, int unreadCount, String extendedProperty) {
        ...
        row.setProperty("receivedDate",      formatDateTime(text(msg, "receivedDateTime")));
        row.setProperty("hasAttachments",    msg.path("hasAttachments").asBoolean(false) ? "Yes" : "No");
        row.setProperty("isRead",            msg.path("isRead").asBoolean(true) ? "Read" : "Unread");
